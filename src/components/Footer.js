  ...
  return (
    <>
      <section className="bg-[#212529] box-border">
        <div className="px-4 mx-auto sm:max-w-[600px] md:max-w-[760px] lg:max-w-[1010px] xl:max-w-[1280px] py-12 min-[767px]:flex">
          {links.map((item, key) => (
              ...
              <ul className="mt-6">
                {item.child.map((item) => (
                  <li className="pt-2 text-[#98A0AA] text-sm" key={key}>
                    <Link href={item.link + item.title}>{item.title}</Link>
                  </li>
                ))}
              </ul>
            </div>
